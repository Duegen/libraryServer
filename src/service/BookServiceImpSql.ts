...
export class BookServiceImpSql implements BookService {
    ...
    async addBook(book: Book): Promise<void> {
        ...
        const [result] = await pool.query<RowDataPacket[]>('SELECT * FROM books WHERE bookId = ?', book._id)
            .catch(() => {
                throw new Error('database connection error')
            });
        if (result.length)
            throw new HttpError(409, `duplicated bookId ${book._id}, book not added`, '@addBook');
        await pool.query(`INSERT INTO books
                          VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    async getBooksByGenre(genre: string): Promise<Book[]> {
        return await this.getBooks(`SELECT *
                                    FROM books
                                    WHERE genre = '${genre}'`, '@getBooksByGenre');
    }

    async getBooksByAuthor(author: string): Promise<Book[]> {
        return await this.getBooks(`SELECT *
                                    FROM books
                                    WHERE author = '${author}'`, '@getBooksByAuthor');
    }

    async removeBook(bookId: string): Promise<Book> {
        const pool = booksDatabase as Pool;
        const [result] = await pool.query<RowDataPacket[]>('SELECT * FROM books WHERE bookId = ?', bookId)
            .catch(() => {
